...
export default async (req: Request, res: Response, next: NextFunction) => {
  //Authorization: <type> <credentials> 형태이므로 뒤에 <credentials>만 파싱
  ...
  try {
    ...
    const userId: number = (decoded as JwtPayload).userId; // 타입 단언
    ...
    req.body.userId = userId; //근데 왜 res 아니고 req,,,? 다음 미들웨어 입장에는 요청값(req)이기 때문이다 =_=
    next();
  } catch (error) {
    console.log(error);
